(function () {
    ...
    window.db = { ref: function (path) { const tournament = extractTournament(path); if (path === '.info/connected') return { on: function (event, callback) { callback(snapshot(false)); if (configured) fetch(apiUrl(), { method:'POST', headers:{'content-type':'application/json', apikey:anonKey, authorization:`Bearer ${anonKey}`}, body:JSON.stringify({ action:'public', tournament:'healthcheck' }) }).then(response => callback(snapshot(response.ok || response.status === 404))).catch(() => callback(snapshot(false))); } }; return {
