(function () {
    ...
    // Coalesces rapid bursts of realtime updates (e.g. 100+ players scoring at once) into a single
    // re-render every REALTIME_DEBOUNCE_MS instead of re-rendering the whole UI on every DB change.
    ...
    window.db = { ref: function (path) { const tournament = extractTournament(path); if (path === '.info/connected') return { on: function (event, callback) { callback(snapshot(false)); if (configured) fetchWithTimeout(apiUrl(), { method:'OPTIONS', headers:{ apikey:anonKey, authorization:`Bearer ${anonKey}` } }, 5000).then(response => callback(snapshot(response.ok))).catch(() => callback(snapshot(false))); } }; return {
        once: async function () { const data = await call(currentRole === 'admin' && currentTournament === tournament ? 'admin-state' : 'public', { tournament }); return snapshot(data.state); },
        on: function (event, callback) { this.once().then(callback).catch(error => console.warn('Supabase public read unavailable:', error.message)); const throttledCallback = debounce(callback, REALTIME_DEBOUNCE_MS); const channel = subscribeTournament(tournament, throttledCallback); return function unsubscribe() { if (channel && client) { try { client.removeChannel(channel); channels.delete(`tournament-${tournament}`); } catch (error) {} } }; },
